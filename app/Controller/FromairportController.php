...
class FromairportController extends AppController {
	...
	public function index() {
		...
		$airportCd = $landmark['airport_cd'];

		// 空港の投稿を取得
		$airportContents = $this->RcPostmeta->getAirportPostmetaDataByAirportCd($airportCd, $link_cd, $landmark['prefecture_id']);
		...
		// meta系
		// 軽自動車・コンパクトの最安値がある場合、メタタグとタイトルに入れる
		$lowest_car_price = '';
		if (!empty($typeBestPrices[1]) && !empty($typeBestPrices[1]['bestPrice']) && $typeBestPrices[1]['bestPrice'] < 7000) {
			$lowest_car_price = '¥'.$typeBestPrices[1]['bestPrice'].'~';
		}
		$this->set('title_for_layout', $landmarkName . 'の格安レンタカー比較・予約（乗り捨て可）' . $lowest_car_price . '｜スカイチケット');
		$this->set('description_for_layout', $landmarkName . 'のレンタカーを格安で予約！' . $lowest_car_price . $landmarkName . '周辺で早朝・深夜対応可能、24 時間営業の店舗からレンタカーを簡単に探す！乗り捨てや安いプランの料金比較も簡単！スタッドレス、外車、オープンカーなど豊富な車種をスカイチケットでレンタル！');
		$this->set('keywords', $landmarkName . ',' . $prefectureName .','. $title_prefecture . ",レンタカー,格安,比較,予約,乗り捨て,スカイチケット");
		...
		// パンくずリスト設定
		$progressArr = $this->BreadCrumb->setFromairport($this->action, $landmarkName, $region_link_cd, $pref_link_cd, $link_cd, $regionName, $prefectureName);
