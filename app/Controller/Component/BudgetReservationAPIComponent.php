...
class BudgetReservationAPIComponent extends Component {
	...
	// 予約時送信データ準備
	public function setFrontReservationData($reservationData, $reservationDetailData, $reservationChildSheetData, $reservationPrivilegeData) {
		$Office = ClassRegistry::init('Office');
		$CommodityItem = ClassRegistry::init('CommodityItem');
		$Privilege = ClassRegistry::init('Privilege');
		$Privilege->recursive = -1;
		$CommodityEquipment = ClassRegistry::init('CommodityEquipment');

		// 予約データ
		$rentOffice = $Office->getOfficeCodeAndName($reservationData['rent_office_id']);
		$returnOffice = $Office->getOfficeCodeAndName($reservationData['return_office_id']);
		$commodityInfo = $CommodityItem->getCommodityItemRelations($reservationData['commodity_item_id']);

		$reservation = array(
			'reservation_key'			 => $reservationData['reservation_key'],
			'reservation_status_id'		 => Constant::API_STATUS_RESERVATION,
			'changed_by'				 => Constant::API_CHANGED_BY_USER,
			'rent_datetime'				 => date('Y/m/d H:i:s', strtotime($reservationData['rent_datetime'])),
			'rent_shop_id'				 => $rentOffice['Office']['office_code'],
			'rent_shop_name'			 => $rentOffice['Office']['name'],
			'return_datetime'			 => date('Y/m/d H:i:s', strtotime($reservationData['return_datetime'])),
			'return_shop_id'			 => $returnOffice['Office']['office_code'],
			'return_shop_name'			 => $returnOffice['Office']['name'],
			'car_class_id'				 => $commodityInfo['CarClass']['id'],
			'car_class_name'			 => $commodityInfo['CarClass']['name'],
			'car_model_id'				 => $commodityInfo['CarModel']['id'],
			'car_model_name'			 => $commodityInfo['CarModel']['name'],
			'plan_id'					 => $reservationData['commodity_item_id'],
			'plan_name'					 => $commodityInfo['Commodity']['name'],
			'plan_type'					 => ($commodityInfo['Commodity']['sales_type'] == Constant::SALES_TYPE_ARRANGED) ? Constant::API_SALES_TYPE_ARRANGED : Constant::API_SALES_TYPE_AGENT_ORGANIZED,
			'last_name'					 => $reservationData['last_name'],
			'first_name'				 => $reservationData['first_name'],
			'tel'						 => $reservationData['tel'],
			'email'						 => $reservationData['email'],
			'arrival_flight_number'		 => $reservationData['arrival_flight_number'],
			'departure_flight_number'	 => $reservationData['departure_flight_number'],
			'adults_count'				 => $reservationData['adults_count'],
			'children_count'			 => $reservationData['children_count'],
			'infants_count'				 => $reservationData['infants_count'],
			'total_amount'				 => $reservationData['amount'],
		);

		// 予約明細データ
		$details = array();
		foreach ($reservationDetailData as $v) {
			$details[] = array(
				'detail_type_id'	 => $v['detail_type_id'],
				'amount'			 => $v['amount'],
			);
		}
		...
		// 標準オプション（装備）
		$normalOptions = array();
		$equipments = $CommodityEquipment->getEquipmentDataWithName($commodityInfo['Commodity']['id']);
		...
		// 選択オプション（特典）
		$extraOptions = array();
		$privileges = array_merge($reservationChildSheetData, $reservationPrivilegeData);
		foreach ($privileges as $v) {
			$option_id = !empty($v['child_sheet_id']) ? $v['child_sheet_id'] : $v['privilege_id'];
			$privilege = $Privilege->read('name', $option_id);
			$extraOptions[] = array(
				'option_id'		 => $option_id,
				'option_name'	 => $privilege['Privilege']['name'],
				'count'			 => $v['count'],
				'amount'		 => ($v['price'] > 0) ? ($v['price'] / $v['count']) : 0,
			);
		}
	...
	// マイページ送信データ準備
	public function setMypageReservationData($reservationId, $apiStatus) {
	...
	// 予約状態変更
	public function changeApiStatus($apiStatus) {
		if (!empty($this->postData['request']['reservation'])) {
			$this->postData['request']['reservation']['reservation_status_id'] = $apiStatus;
		}
	}
	...
	// アラートメール送信（マイページより）
	public function sendAlertFromMypage($controlNumber, $domain) {
		if (!empty($this->postData['request']['reservation'])) {
			...
			// バジェット向け
			$body = $this->getClientMailBody($controlNumber, $domain, false);
			...
			// スカチケ運用向け
			$subject = sprintf("【skyticket】マイページからの%s連携でエラーが発生しました（%s、%s）", $statusName, $this->clientName, $reservationKey);
			$this->sendAlertMailWithBodyPrefix('', $subject, EMAIL_ADDRESS_RENTACAR);
		}
	}

	// アラートメール送信（予約時）
	public function sendAlertFromFront($controlNumber, $domain) {
		if (!empty($this->postData['request']['reservation'])) {
			$reservationKey = $this->postData['request']['reservation']['reservation_key'];

			// バジェット向け
			$body = $this->getClientMailBody($controlNumber, $domain, true);
			$subject = sprintf("【skyticket】予約連携でエラーが発生しました（%s）", $reservationKey);
			$this->sendAlertMail($body, $subject, IS_PRODUCTION ? Constant::BUDGET_ERROR_EMAIL : EMAIL_ADDRESS_SYS);

			// スカチケ運用向け
			$subject = sprintf("【skyticket】予約連携でエラーが発生しました（%s、%s）", $this->clientName, $reservationKey);
			$this->sendAlertMailWithBodyPrefix('', $subject, EMAIL_ADDRESS_RENTACAR);
		}
	}
	...
	// バジェットに送るエラー通知メールの本文
	private function getClientMailBody($controlNumber, $domain, $isReserve) {
		// メッセージ
		$body = $this->getClientMailMessage($isReserve);

		// 予約内容
		$body .= $this->getClientMailData($controlNumber);
	...
	private function getClientMailMessage($isReserve) {
		$body = '';

		if ($isReserve) {
			$body .= '予約連携でエラーが発生しました。' . PHP_EOL;
			$body .= '予約内容（下記）をご確認の上、該当の予約が存在する場合はキャンセルして下さい。' . PHP_EOL;
			$body .= PHP_EOL;
		} else {
	...
	// ほぼ notification と同じ
	private function getClientMailData($controlNumber) {
		$body = '';
