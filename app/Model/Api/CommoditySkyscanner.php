...
final class CommoditySkyscanner extends BaseCommodityMetasearch {
	public $actsAs = array('CommoditySkyscanner');
	...
	public function paginate($conditions, $fields, $order, $limit, $page = 1, $recursive = null, $extra = array()) {
		...
		// SIPPコード取得
		$SippCodeLetter = new SippCodeLetter();
		$letters = $SippCodeLetter->getSippCodeLettersList();
		...
		foreach ((array)$commodities as $k => $commodity) {
			foreach ((array)$ret as $ret_k => $ret_v) {
				...
				$sipp_code = $commodity['sippCode'];
				$car_model = $carInfo['CarModel'][0];

				$plan_info = array(
					'plan_id'			 => $commodity['commodityItemId'],
					'plan_name'			 => $ret_v['Commodity']['name'],
					'car_type'			 => $this->getCarType($letters, $sipp_code),
					'car_model'			 => $this->getCarModelName('・', $carInfo['CarModel']),
					'car_img_url'		 => $img_url . $ret_v['Commodity']['client_id'] . '/' . $ret_v['Commodity']['image_relative_url'],
					'sipp_code'			 => $sipp_code,
					'car_capacity'		 => $car_model['capacity'],
					'car_door'			 => $car_model['door'],
					'transmission'		 => $this->getTransmission($ret_v['Commodity']['transmission_flg']),
					'fuel_type'			 => $this->getFuelType($sipp_code),
					'ac'				 => true, // true固定
					'baggage'			 => $car_model['trunk_space'],
					'currency'			 => 'jpy',
				...
				$option_list = array();
				$option_list[] = array(
					'option_id'			 => $optionCategories[$smoking_id]['id'],
					'option_name'		 => $this->getOptionName($optionCategories[$smoking_id]['id']),
					'standard_equipment' => '1',
					'option_num'		 => '1',
					'option_price'		 => '0',
				);
				...
				foreach ($options as $option) {
					...
					$option_name = $this->getOptionName($option_category['id']);

					if (!empty($option_name)) {
						$option_list[] = array(
							'option_id'			 => $option_category['id'],
							'option_name'		 => $option_name,
							'standard_equipment' => $option['option_default'],
							'option_num'		 => '1',
				...
				// 付加価値をセットする
				$plan_info['value_adds'] = array(
					'fuel_policy'				 => 'full_to_full',
					'shuttle_service'			 => $this->getShuttleService($commodity['canPickup']),
					'one_way_surcharges'		 => !empty($commodity['dropOffCharge']),
					'free_cancellation'			 => true,
					'free_amendment'			 => true,
					'young_driver_surcharge'	 => 0,
					'free_breakdown_assistance'	 => true,
					'theft_protection'			 => true,
					'third_party_cover'			 => true,
					'collision_damage_waiver'	 => true,
					'free_additional_driver'	 => true,
					'additional_driver_price'	 => 0,
					'additional_driver_currency' => 'jpy',
					'unlimited_mileage'			 => true,
					'distance_limitation'		 => false,
					'mandatory_chauffeur'		 => false,
				);

				// ディープリンクを生成する
				$plan_info['deeplink'] = $this->getPlanUrl($commodity['commodityItemId']);
...
//					$searchConditions['smoking_flg'], 2
		...
		$basicConditions = array(
			...
			'Commodity.sales_type' => Constant::SALES_TYPE_ARRANGED, // 一応条件追加するが、スカイスキャナー再開の可能性はないor薄いため、動作確認はしない
			'CommodityItem.sipp_code IS NOT NULL',
