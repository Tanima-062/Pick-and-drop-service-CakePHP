  ...
  <div class="review_container">
    <div class="review_container_header">
      <?php if($reviewCount >= $yotpoReviewLimit){ ?>
        <div class="review_count">全<?=$reviewCount;?>件<br><span style="font-weight:400;font-size:14px;">(最新<?=$yotpoReviewLimit;?>件を表示)</span></div>
      <?php }else{ ?>
        ...
        if(avgScore > i) avgStars += '<i class="icm-star-full"></i>'
          ...
          if((i - avgScore) < 1) { 
            avgStars += `<div class="rate-score">
                          <i class="icm-star-full" style="width: ${(avgScore - (i - 1)) * 18}px"></i>
                          <i class="icm-star-empty"></i>
                        </div>`
          }
