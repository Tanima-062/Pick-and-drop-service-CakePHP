...
class BudgetReservationAPIComponent extends Component {
	...
	// 管理画面送信データ準備
	public function setClientReservationData($reservationId, $apiStatus) {
	...
	// アラートメール送信（管理画面より）
	public function sendAlertFromClient($controlNumber, $domain) {
		if (!empty($this->postData['request']['reservation'])) {
			...
			// バジェット向け
			$body = $this->getClientMailBody($controlNumber, $domain);
	...
	// バジェットに送るエラー通知メールの本文
	private function getClientMailBody($controlNumber, $domain) {
		// メッセージ
		$body = $this->getClientMailMessage();
	...
	private function getClientMailMessage() {
