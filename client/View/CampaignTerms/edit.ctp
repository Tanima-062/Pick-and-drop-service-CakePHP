...
<?php $term_count = $this->request->data['Campaign']['termCount']; ?>
<?php echo $this->Form->hidden('Custom.referer', array('value' => $referer)); ?>
<h3><?php echo __('キャンペーン期間編集'); ?></h3>
...
<?php for ($i = 0; $i < $term_count; $i++) { ?>
	...
	?>
		<tr class="term_add">
			...
			<?php echo $this->Form->hidden("CampaignTerm.{$i}.id"); ?>
		</tr>
<?php } ?>
		<tr>
			<th>公開範囲</th>
			<td><?php echo $this->Form->select('scope', $scopeList); ?></td>
		</tr>
		<tr>
			<th>削除</th>
