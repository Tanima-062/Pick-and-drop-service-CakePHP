...
<div style="margin-bottom: 20px;">
	...
	<table class="table table-bordered">
		...
		<tr>
			...
			<td>
			<?php echo $this->Form->input("CommodityPrice.{$commoditySchedule}.commodity_price", array(
					'type' => 'number',
					'label' => false,
					'min' => 0,
					'required' => true,
			));
			echo $this->Form->hidden("CommodityPrice.{$commoditySchedule}.id");
			?>
			</td>
			<?php } ?>
		</tr>
...
<table id="campaign_list" class="table table-bordered">
	...
	<tr id="CommodityCampaignPrice<?php echo $id; ?>">
		...
		<?php } ?>
		<td><?php echo $this->Form->select("CommodityCampaignPrice.{$id}.delete_flg", $isPublishedOptions, array('empty' => false, 'class' => 'span5 campaign_delete', 'style' => 'width:85px')); ?></td>
	</tr>
	<?php } ?>
</table>
