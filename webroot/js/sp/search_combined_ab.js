// SP ABテストのパターンの時に使う「条件変更」＋「絞り込み」合わせられたモーダルで使う

$(function () {
    var $form = $('#SearchIndexForm');
                ...
                case 1:
                    ...
                    if ($form.find("#prefecture").val() == 0) {
                      $form.find("#area_placeholder").show();
                        ...
                        setArea($form.find("#prefecture").val(), 1);
                    }
                    break;
                case 4:
                    $form.find("#js-prefectureOptions")
                    ...
                    if ($form.find("#prefecture").val() == 0) {
                        ...
                        setStation($form.find("#prefecture").val(), 1);
                    }
                    break;
                default:
                    break;
    ...
    // returnWay
    $form.on("change", "#return_way_check", function () {
        ...
        switch (radio_val) {
            case 1:
                ...
                if ($form.find("#return_prefecture").val() == 0) {
                    ...
                    setArea($form.find("#return_prefecture").val(), 0);
                }
                break;
            case 4:
                $form.find("#js-returnPrefectureOptions")
                ...
                if ($form.find("#return_prefecture").val() == 0) {
                    ...
                    setStation($form.find("#return_prefecture").val(), 0);
                }
                break;
            default:
                break;
    ...
    setDateValue($form.find("#SearchDate"));
    setDateValue($form.find("#SearchReturnDate"));

    var return_way_val = $form.find("input[name=return_way]:checked").val();
    if (return_way_val > 0) {
        $form.find("#return_way_check").prop("checked", false);
        $form.find(".search_select_return").show();
        $form.find(".tab_return[data-radio=" + return_way_val + "]").trigger("click");
    } else {
        $form.find("#return_way_check").trigger("change");
    }
})
...
function checkRange() {
    var $form = $('#SearchIndexForm');
    $form.find("#js-searchform_error").show();
    const returnFlg = $form.find("#return_way_check").prop("checked");
    let val = null;
    $form.find("#js-searchform_error").html("");
