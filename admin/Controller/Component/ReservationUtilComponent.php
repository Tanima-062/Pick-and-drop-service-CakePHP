...
class ReservationUtilComponent extends PlanUtilComponent
{
	...
	public function sendNotificationMail($reservationId, $status)
	{
		...
		$reservation = $Reservation->getReservationDataForMail($reservationId);
		...
		$clientEmail = $ClientEmail->getClientEmail($reservation['Client']['id']);
		foreach ($clientEmail as $val) {
			if (!empty($val['ClientEmail']['reservation_email'])) {
				$email->to(trim($val['ClientEmail']['reservation_email']));
				$email->send();
	...
	public function priceCalculation(
		...
		$fromOfficeId,
		$returnOfficeId,
		$estimationTotalPrice,
		$optionSheet = array(),
		$optionPrivilege = array()
	) {
	 ...
	 * @return int|boolean
	 */
	public function priceCalculationAgentOrganized(
		$commodityItemId,
		$fromDateTime,
		$toDateTime,
		$fromOfficeId,
