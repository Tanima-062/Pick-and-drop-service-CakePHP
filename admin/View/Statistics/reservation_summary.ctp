...
期間　<?php echo $this->Form->select('segment', array('月別', '日別', '時間別'), array('default' => '0')); ?>
...
if (empty($this->request->data['Reservation']['segment'])) {
	// 月別
	echo $this->element('Statistics/reservation_summary_monthly');
} else if ($this->request->data['Reservation']['segment'] == '1') {
	// 日別
	echo $this->element('Statistics/reservation_summary_daily');
} else if ($this->request->data['Reservation']['segment'] == '2') {
	// 時間別
	echo $this->element('Statistics/reservation_summary_hourly');
}
?>