...
<script>
	...
	$(document).on('ready', function(){
		$('input[name="data[CancelFee][is_after_departure]"]:eq(0)').prop('checked', true);
		$('input[name="data[CancelFee][cancel_fee_unit]"]:eq(0)').prop('checked', true);
		$('#fraction_show').hide();
		$('#cancel_fee_min_show').hide();
		$('#cancel_fee_max_show').hide();
...
<div class="cancelFees form span8">
<?php
	...
	<table class="table table-bordered form-inline">
		...
		<tr>
			...
			<td><?php echo $this->Form->input('sales_type',[
					...
					'value' => Constant::SALES_TYPE_ARRANGED,
					'div' => false
				]);
				?>
			</td>
		...
		<tr>
			<th>適用期間</th>
			<td>
				<span class='input-prepend'><?php echo $this->Form->input('apply_term_from', ['type' => 'text', 'required' => true, 'value' => '']); ?></span>&nbsp;〜&nbsp;
				<span class='input-append add-on'><?php echo $this->Form->input('apply_term_to', ['type' => 'text', 'required' => true, 'value' => '']); ?></span>
			</td>
		</tr>
		<tr>
			<th>出発前・出発後</th>
